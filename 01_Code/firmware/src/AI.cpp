...
const char* allPlaceTypes[] = {
	...
	"museum","sculpture","amphitheatre","amusement_center","amusement_park",
	"botanical_garden","cultural_center","dog_park","ferris_wheel","garden","hiking_area",
	"historical_landmark","marina","national_park","observation_deck","opera_house",
	"park","picnic_ground","planetarium","plaza","tourist_attraction","wildlife_park","wildlife_refuge","zoo"
};
...
String queryGemini(String weatherCondition, String localTime) {
	...
	for (int i = 0; i < sizeof(allPlaceTypes)/sizeof(allPlaceTypes[0]); i++) {
		...
		if (i < (sizeof(allPlaceTypes)/sizeof(allPlaceTypes[0]) - 1)) prompt += ", ";
	}
	prompt.replace("\"", "\\\"");
