...
{
public:
    // Construction: Pass in the hardware serial port instance and the comparison interval (milliseconds)
    ...
    const MoveReport &lastReport() const { return report_; }

    // Print heartbeat status (optional)
    void heartbeat(Stream &out, uint32_t everyMs = 2000);

    // Expose TinyGPSPlus directly to facilitate access to additional fields when required (optional)
